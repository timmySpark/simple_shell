...
int cases(cmd_t *cmds, cmd_t *args, int cmd_count, int i, char *argv)
{
	...
	for (j = 0; j < sizeof(commands) / sizeof(commands[0]); j++)
	{
		if (_strncmp(cmd_name, commands[j].name,
			...
			return (commands[j].func(args, cmds,
						cmd_count, i, argv));
	}
	return (0);
}
...
int _unsetenv_cmd(cmd_t *args, cmd_t *cmds,
		int cmd_count, int i, char *argv)
{
...
/**
 * _exit_cmd - handle exit commands
 * @cmds: commands
 * @args: arguments
 * @cmd_count: Number of commands executed
...
int _exit_cmd(cmd_t *args, cmd_t *cmds,
		int cmd_count, int i, char *argv)
{
...
/**
 * _env_cmd - handle environment commands
 * @cmds: commands
 * @args: arguments
 * @cmd_count: Number of commands executed.
 * @i: i command in cmds.
...
int _env_cmd(cmd_t *args, cmd_t *cmds,
		int cmd_count, int i, char *argv)
{
	(void)cmds;
	(void)cmd_count;
